...
class ViewerSelectionItem(QGraphicsItem):
    ...
    def cropValues(self):
        ...
        def cV(r):
            left_ratio = (r.left() - p.left()) / p.width()
            top_ratio = (r.top() - p.top()) / p.height()
            right_margin_ratio = 1.0 - ((r.right() - p.left()) / p.width())
            bottom_margin_ratio = 1.0 - ((r.bottom() - p.top()) / p.height())
            return (
                clamp(left_ratio),
                clamp(top_ratio),
                clamp(right_margin_ratio),
                clamp(bottom_margin_ratio),
            )
