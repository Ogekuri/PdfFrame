...
class SemiAbstractPdfFrameper(AbstractPdfFrameper):
    """An extension of AbstractPdfFrameper that implements the basic logic
...
class PyPdfFrameper(SemiAbstractPdfFrameper):
    """Implementation of PdfFrameper using pypdf"""
    def __init__(self):
...
class PyPdfOldCropper(PyPdfFrameper):
    """Implementation of PdfFrameper using PyPDF2 (<2)"""
    def doAddPage(self, page, rotate):
        if rotate != 0:
...
class PyMuPdfFrameper(SemiAbstractPdfFrameper):
    """Implementation of PdfFrameper using PyMuPDF"""
    def __init__(self):
...
class PikePdfFrameper(SemiAbstractPdfFrameper):
    """Implementation of PdfFrameper using pikepdf"""
    def __init__(self):
...
def import_pymupdf():
    ...
    PyMuPdfFrameper.pymupdf = fitz
    return PyMuPdfFile, PyMuPdfFrameper

def import_pikepdf():
    from pikepdf import Pdf
    PikePdfFile.Pdf = Pdf
    PikePdfFrameper.Pdf = Pdf
    return PikePdfFile, PikePdfFrameper

def import_pypdf():
    from pypdf import PdfReader, PdfWriter
    PyPdfFile.PdfReader = PdfReader
    PyPdfFrameper.PdfWriter = PdfWriter
    return PyPdfFile, PyPdfFrameper
...
def import_pypdf2():
    ...
    if PyPDF2.__version__.startswith("1."):
        ...
        PdfFrameper = PyPdfOldCropper
    else:
        from PyPDF2 import PdfReader, PdfWriter
        PdfFile = PyPdfFile
        PdfFrameper = PyPdfFrameper
    PdfFile.PdfReader = PdfReader
    PdfFrameper.PdfWriter = PdfWriter
    return PdfFile, PdfFrameper
...
for load_only_if_requested in [True, False]:
    for lib, import_func, flag in lib_crop_options:
        if not lib_crop and (not load_only_if_requested or (flag and flag in sys.argv)):
            try:
                PdfFile, PdfFrameper = import_func()
                lib_crop = lib
