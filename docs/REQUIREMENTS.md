...
### 1.2 Project Scope
PdfFrame implements PDF page-region cropping through a Qt GUI with optional command-line preconfiguration and batch execution, computes crop parameters from GUI state, and performs final cropping through PyMuPDF page manipulation.
...
| Crop/render engine (PyMuPDF) | `src/pdfframe/pdfframecmd.py` uses `fitz` API for CropBox/MediaBox manipulation, annotation deletion, and bookmark preservation; `src/pdfframe/vieweritem.py` uses `fitz` for page rendering and geometry. |
| Rendering fallback (PopplerQt5) | `src/pdfframe/vieweritem.py` optionally imports `popplerqt5.Poppler` for PyQt5 page rendering. |
...
- **PRJ-003**: MUST map CLI options to GUI state before execution, including selection construction controls, optional `--whichpages` page filtering, crop-parameter controls used to derive crop geometry, and runtime logging flags.
...
- **CTN-003**: MUST execute final cropping through PyMuPDF `fitz` API page manipulation and MUST NOT invoke external `gs` subprocess.
- **CTN-004**: MUST raise a user-visible error when PyMuPDF crop/write operations fail with an exception.
- **CTN-005**: MUST use PyMuPDF for rendering/geometry metadata and for crop/frame output generation.
...
- **DES-001**: MUST implement `MainWindow` as an orchestration layer connecting UI events to `ViewerItem`, `ViewerSelections`, PyMuPDF crop execution, and `autoTrimMargins`.
...
- **DES-007**: MUST execute cropping by translating GUI selections into PyMuPDF CropBox/MediaBox operations, processing pages iteratively through `fitz` API, and reporting per-page progress to the UI callback.
- **DES-008**: MUST route warnings to `QMessageBox` when the window is visible and to `stderr` when the window is hidden.
- **DES-009**: MUST report per-page progress during PyMuPDF crop processing via callback and MUST NOT display internal exception details in user-visible failure messages.
...
- **REQ-008**: MUST process only pages selected by `--whichpages` (or all pages when empty), compute crop parameters from the primary GUI selection only, and execute PyMuPDF crop operations for the selected page range.
...
- **REQ-010**: MUST apply PyMuPDF CropBox for `frame` mode (visible area restricted to selection bounds on original-size page) and CropBox plus MediaBox for `crop` mode (page physically resized to selection bounds).
...
- **REQ-015**: MUST open a progress window during conversion, report per-page progress during PyMuPDF crop processing, and update a progress bar from `0` to total pages selected for processing.
...
- **REQ-017**: MUST expose a stop button in the conversion progress window that allows users to cancel the active PyMuPDF crop processing before completion.
...
- **REQ-019**: MUST print PyMuPDF crop operation parameters before conversion execution when both `--verbose` and `--debug` are enabled.
...
- **REQ-022**: MUST expose CLI flags `--verbose` and `--debug`; `--verbose` enables Python-side console progress/status output including renderer-selection diagnostics, and `--verbose --debug` additionally prints PyMuPDF crop parameters and per-page processing output.
...
- **REQ-032**: MUST expose checked-by-default `Delete annotations fields` in `Extra operations on the final PDF` and, when enabled, remove all `/Annots` entries from every output page using native PyMuPDF annotation API.
- **REQ-033**: MUST update Help-tab and UI explanatory text to reflect active Basic-tab controls and MUST NOT reference `Rotation`, `Use Ghostscript to optimize`, `Include pages without selections`, or Ghostscript.
...
- **REQ-038**: MUST preserve all PDF bookmarks (table of contents) from the source document in the output PDF when bookmarks are present.
...
- **TST-003**: MUST include unit tests that validate PyMuPDF crop/frame page processing for `frame` and `crop` modes from GUI-derived crop data.
- **TST-004**: MUST include unit tests that validate failure diagnostics omit internal exception details from user-visible warnings when crop operations fail and that per-page progress callbacks drive progress updates.
- **TST-005**: MUST include unit tests that validate large page-index support, allowed single-range `--whichpages` formats, and primary-selection-only crop planning for PyMuPDF range conversion.
- **TST-006**: MUST include unit tests that validate PyMuPDF crop operation logging/output toggles (`--verbose`, `--debug`) and desktop integration branding identifiers (`pdfframe` / `com.ogekuri.pdfframe`).
...
- **TST-011**: MUST include unit tests that validate `Delete annotations fields` default-checked UI placement and PyMuPDF `/Annots` removal behavior when enabled in both `frame` and `crop` modes.
...
| --- | --- |
| PRJ-001 | `src/pdfframe/mainwindow.py::slotPdfFrame` creates and updates a progress dialog while `crop_pdf_pages(...)` executes conversion. |
...
| CTN-003 | `src/pdfframe/mainwindow.py::slotPdfFrame` and `src/pdfframe/pdfframecmd.py::crop_pdf_pages` — output crop execution occurs through PyMuPDF `fitz` API; no `gs` subprocess is invoked. |
| CTN-004 | `src/pdfframe/mainwindow.py::slotPdfFrame` — catches `CropError` and `CropCancelledError` from `crop_pdf_pages` and displays user-visible diagnostics. |
| CTN-005 | `src/pdfframe/vieweritem.py` — PyQt6 branch requires `fitz`; PyQt5 branch tries `fitz` then `popplerqt5`; `src/pdfframe/pdfframecmd.py::crop_pdf_pages` uses `fitz` for crop output generation. |
...
| DES-001 | `src/pdfframe/mainwindow.py::MainWindow` imports/uses `ViewerItem`, `ViewerSelections`, PyMuPDF crop helpers, and `autoTrimMargins`. |
...
| DES-007 | `src/pdfframe/mainwindow.py::buildCropPlan/slotPdfFrame` plus `src/pdfframe/pdfframecmd.py::crop_pdf_pages` — constructs crop plan with page indexes and crop box, iterates pages through PyMuPDF `fitz` API, and reports per-page progress to UI callback. |
...
| DES-009 | `src/pdfframe/pdfframecmd.py::crop_pdf_pages` reports per-page progress via callback and `src/pdfframe/mainwindow.py::slotPdfFrame` uses callback to advance progress while keeping internal error details out of user-visible warnings. |
...
| REQ-003 | `src/pdfframe/viewerselections.py::selectionVisibleOnPage` and `src/pdfframe/mainwindow.py::buildCropPlan` apply one GUI selection uniformly to processed pages without mode-dependent branching. |
...
| REQ-008 | `src/pdfframe/mainwindow.py::buildCropPlan` processes only requested pages, derives geometry from the primary selection tuple, and produces a crop plan with page indexes and crop box for PyMuPDF processing. |
...
| REQ-010 | `src/pdfframe/pdfframecmd.py::crop_pdf_pages` applies PyMuPDF CropBox for `frame` mode and CropBox+MediaBox for `crop` mode. |
...
| REQ-015 | `src/pdfframe/mainwindow.py::slotPdfFrame` uses per-page progress callbacks from `src/pdfframe/pdfframecmd.py::crop_pdf_pages` to advance progress bar. |
| REQ-016 | `pdfframe.sh` resolves repository root and runs `uv run --project <repo> pdfframe ...` to execute the GUI through Astral `uv`. |
| REQ-017 | `src/pdfframe/mainwindow.py::slotPdfFrame` checks progress dialog cancel state via `cancel_requested` callback passed to `crop_pdf_pages`. |
...
| REQ-019 | `src/pdfframe/mainwindow.py::slotPdfFrame` passes verbose/debug flags to `src/pdfframe/pdfframecmd.py::crop_pdf_pages(log_params=..., debug_output=...)`, which conditionally emits crop parameters. |
...
| REQ-022 | `src/pdfframe/application.py::main` defines `--verbose`/`--debug`; `src/pdfframe/vieweritem.py` gates renderer-selection diagnostics by `--verbose`; `src/pdfframe/mainwindow.py::slotPdfFrame` and `src/pdfframe/pdfframecmd.py::crop_pdf_pages` enforce verbose/debug PyMuPDF logging behavior. |
...
| REQ-032 | `src/pdfframe/mainwindow.ui` and `src/pdfframe/mainwindow.py::readSettings/buildCropPlan` expose `Delete annotations fields` UI and `src/pdfframe/pdfframecmd.py::crop_pdf_pages` removes `/Annots` via PyMuPDF when enabled. |
...
| REQ-038 | `src/pdfframe/pdfframecmd.py::crop_pdf_pages` preserves PDF bookmarks (TOC) in output via PyMuPDF `doc.save(...)`. |
...
| TST-003 | `tests/test_pdfframecmd.py` validates PyMuPDF crop/frame page processing for `frame` and `crop` modes. |
| TST-004 | `tests/test_mainwindow_conversion_errors.py` and `tests/test_mainwindow_progress_updates.py` validate warning payload redaction and per-page progress callbacks that drive progress updates. |
...
| TST-006 | `tests/test_pdfframecmd.py` and `tests/test_mainwindow_progress_updates.py` validate crop operation logging/output toggles; `tests/test_desktop_metadata_branding.py` validates desktop/AppStream branding identifiers. |
...
| TST-011 | `tests/test_mainwindow_preserve_fields.py` and `tests/test_pdfframecmd.py` validate default-checked `Delete annotations fields` UI placement and PyMuPDF `/Annots` removal behavior in both conversion modes. |
...
Repository unit tests currently target the PyMuPDF crop backend helper module (`tests/test_pdfframecmd.py`), while GUI interaction behavior remains covered by runtime/manual verification paths.
