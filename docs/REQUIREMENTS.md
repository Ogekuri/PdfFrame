...
- **REQ-004**: MUST auto-trim selection edges when grayscale changes remain within sensitivity and allowed-change thresholds configured through the dedicated Basic-tab trim settings section.
...
- **REQ-007**: MUST show warnings and use safe defaults when grid or trim-settings values (padding, allowed changes, sensitivity) are invalid.
...
| REQ-004 | `src/pdfframe/mainwindow.py::trimMarginsSelection` reads trim thresholds from Basic-tab controls and `src/pdfframe/autotrim.py::autoTrimMargins` applies them to edge trimming. |
...
| REQ-007 | `src/pdfframe/mainwindow.py::createSelectionGrid/getPadding/trimMarginsSelection` — invalid grid or trim-settings values trigger `showWarning(...)` and fallback defaults. |
