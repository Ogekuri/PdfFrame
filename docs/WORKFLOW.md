...
- Internal Call-Trace Tree:
  - `main(...)`: parse CLI options, configure runtime flags, construct UI, dispatch startup actions, and enter Qt loop [`src/pdfframe/application.py`]
    ...
    - `MainWindow.openFile(...)`: open source PDF, initialize output path, enable conversion actions [`src/pdfframe/mainwindow.py`]
    ...
    - `MainWindow.slotPdfFrame(...)`: build conversion plan, open progress dialog, execute Ghostscript, and handle error/cancel paths [`src/pdfframe/mainwindow.py`]
      ...
      - `MainWindow.buildGhostscriptCropPlan(...)`: derive one Ghostscript command for selected range [`src/pdfframe/mainwindow.py`]
        ...
        - `build_ghostscript_page_crop_command(...)`: build Ghostscript argument vector [`src/pdfframe/pdfframecmd.py`]
