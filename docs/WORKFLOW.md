...
- Lifecycle/trigger:
  ...
  - Conversion action `actionPdfFrame` is exposed in the UI with label `Go!`, is positioned at the right edge after `Save Margins`, and uses a dedicated bundled toolbar icon asset.
...
- Internal Call-Trace Tree:
  - `main(...)`: parse CLI args, initialize Qt app/window, map verbose/debug runtime flags, and dispatch startup actions [`src/pdfframe/application.py`]
    - `MainWindow.__init__(...)`: wire UI actions, construct scene/viewer, and load persisted settings [`src/pdfframe/mainwindow.py`]
      ...
      - `_toolbarIconFromAsset(...)`: resolve bundled toolbar icon assets for `Trim Margins`/`Save Margins`/`Go!` with theme fallback tokens [`src/pdfframe/mainwindow.py`]
      - `_setupTrimPresetAction(...)`: add toolbar `Save Margins` action adjacent to `Trim Margins`, bind dedicated `Save Margins` icon asset, reposition `Go!` at the right edge after `Save Margins`, and connect preset-save slot [`src/pdfframe/mainwindow.py`]
    ...
    - `MainWindow.slotSaveMarginsPreset(...)`: snapshot current mode/trim/crop state into a new timestamp-named preset and persist JSON [`src/pdfframe/mainwindow.py`]
      ...
      - `MainWindow._persistTrimPresetDocument(...)`: write current runtime config plus presets array [`src/pdfframe/mainwindow.py`]
        - `JsonConfigStore.save(...)`: persist normalized JSON config payload [`src/pdfframe/jsonconfig.py`]
    - `MainWindow.createSelectionGrid(...)`: optionally initialize one startup selection area from `--grid` while reducing multi-cell requests to one area [`src/pdfframe/mainwindow.py`]
      - `ViewerSelections.addSelection(...)`: create the first selection item or return the existing item in single-selection mode [`src/pdfframe/viewerselections.py`]
      - `ViewerSelectionItem.setBoundingRect(...)`: apply normalized bounds to the single selection area [`src/pdfframe/viewerselections.py`]
        - `ViewerSelectionItem.adjustBoundingRect(...)`: clamp and normalize rectangle updates [`src/pdfframe/viewerselections.py`]
    - `ViewerSelections.mousePressEvent(...)`: create selection by drag only when no area exists; otherwise keep the existing area current [`src/pdfframe/viewerselections.py`]
    - `MainWindow.slotPdfFrame(...)`: execute crop pipeline (scheduled by `--go` or invoked by UI action) [`src/pdfframe/mainwindow.py`]
      ...
      - `MainWindow.buildGhostscriptCropPlan(...)`: translate GUI selections to one Ghostscript command plan over the selected page range and propagate `Preserve fields` toggle state [`src/pdfframe/mainwindow.py`]
        ...
        - `AbstractViewerItem.cropValues(...)`: collect normalized crop regions for current page [`src/pdfframe/vieweritem.py`]
          - `ViewerSelections.cropValues(...)`: aggregate crop values from the single selection area [`src/pdfframe/viewerselections.py`]
        ...
        - `build_ghostscript_page_crop_command(...)`: build script-style Ghostscript subprocess vector with `-dFirstPage/-dLastPage` and `-dPreserveAnnots=true/false` for `frame`/`crop` modes [`src/pdfframe/pdfframecmd.py`]
    ...
    - `MainWindow.slotTrimMarginsAll(...)`: trim the currently visible single selection area on the active page [`src/pdfframe/mainwindow.py`]
      - `MainWindow.trimMarginsSelection(...)`: compute auto-trim rectangle using current page by default or visible pages inside configured `checkTrimPagesRange`/`editTrimPagesRange` bounds when range mode is enabled, apply color-sensitivity/grayscale-sensitivity thresholds, add configured padding, and clamp to page-image bounds (fallback to selection bounds when page-image geometry is unavailable) [`src/pdfframe/mainwindow.py`]
